...
`;class Oi extends b{constructor(e){super(e),this.props.events={click:this.props.onClick||(()=>{})}}render(){return Ii}}const Ai=`
...
`;class Ei extends b{constructor(e){super(e),this.props.onBlur=()=>this._isValid,this.props.validate=this.props.validate||(()=>!0)}get _value(){return this.refs.input.element.value}get value(){if(this._isValid)return this._value;console.error("Валидация не пройдена")}get _isValid(){return this.props.validate(this._value,this.refs.errorText)}render(){return Ai}}const Ni=`
...
`;class Bi extends b{constructor(e){super(e),this.props.events={submit:this.props.onSubmit||(r=>{r.preventDefault()})}}get values(){if(Object.keys(this.refs).length===1)return Object.entries(this.refs)[0][1].value;let e=!1;const r=Object.entries(this.refs).reduce((s,[c,p])=>(p.value||(e=!0),s[c]=p.value,s),{});return e?void 0:r}render(){return Ni}}const Di=`
  <p class='error {{ className }}' ref='{{ ref }}'>{{ text }}</p>
`;class Ti extends b{render(){return Di}}const Ri=`
...
`;class Fi extends b{render(){return Ri}}const Ui=`
  <div class='content'>
...
`,D=(t,e)=>class extends t{constructor(r){let s=e(S.state);super({...r,...s}),this._onChangeStoreCb=()=>{const c=e(S.state);pr(s,c)||(this.setProps({...c}),s=dr(c))},S.on(Pe.Updated,this._onChangeStoreCb)}componentWillUnmount(){super.componentWillUnmount(),S.off(Pe.Updated,this._onChangeStoreCb)}};class Hi extends b{render(){return Ui}}const Vi=t=>({search:t.search}),qi=D(Hi,Vi),zi=`
...
`;class Wi extends b{render(){return zi}}const Gi=`
  <div class='info'>
    {{# unless search }}
      {{# if time }}
...
`;class $i extends b{render(){return Gi}}const Ki=t=>({search:t.search}),Ji=D($i,Ki),Qi=`
...
`;class ji extends b{constructor(e){super(e),this.props.onUpload=()=>{const[r]=document.querySelectorAll("#add_file");r.click()},this.props.onRemove=()=>{}}render(){return Qi}}const Yi=`
  <input
    ...
    value={{ value }}
  >
`;class Zi extends b{constructor(e){super({...e,events:{blur:e.onBlur||(()=>{})}})}render(){return Yi}}const Xi=`
...
`;class eo extends b{render(){return Xi}}const to=t=>{var e;return{users:(e=t.chat)==null?void 0:e.users}},ro=D(eo,to),so=`
...
`;class no extends b{render(){return so}}const io=`
...
`;class ho extends b{render(){return uo}}const po=t=>{var e;return{userAvatar:(e=t.user)==null?void 0:e.avatar}},fo=D(ho,po),mo=`
...
`;class go extends b{render(){return mo}}const vo=t=>{var e;return{messages:(e=t.chat)==null?void 0:e.messages}},_o=D(go,vo),yo=`
...
`,Q=new ft("chats");class wo{get(e){return Q.get("/",{data:e})}create(e){return Q.post("/",{data:e})}delete(e){return Q.delete("/",{data:{chatId:e}})}getUsers(e){return Q.get(`/${e}/users`,{})}uploadAvatar(e,r){return r.append("chatId",e.toString()),Q.put("/avatar",{data:r})}addUsers(e){return Q.put("/users",{data:e})}deleteUsers(e){return Q.delete("/users",{data:e})}getToken(e){return Q.post(`/token/${e}`,{})}}const G=new wo,oe=new ft("user");class Co{changeProfile(e){return oe.put("/profile",{data:e})}changeAvatar(e){return oe.put("/profile/avatar",{data:e})}changePassword(e){return oe.put("/password",{data:e})}get(e){return oe.get("/",{data:{id:e}})}search(e){return oe.post("/search",{data:{login:e}})}}const we=new Co,So="wss://ya-praktikum.tech/ws/chats";class Po{constructor({chatId:e,token:r}){var s;this._userId=((s=S.state.user)==null?void 0:s.id)||0,this._chatId=e,this._token=r,this.socket=this._createConnection(),this._start()}_createConnection(){return new WebSocket(`${So}/${this._userId}/${this._chatId}/${this._token}`)}_start(){this.socket.addEventListener("open",()=>{console.log(`${this._chatId}: Соединение установлено`),this._pingPong.bind(this)}),this.socket.addEventListener("close",e=>{e.wasClean?console.log("Соединение закрыто чисто"):(console.log("Обрыв соединения"),this.socket=this._createConnection(),this._start()),console.log(`Код: ${e.code} | Причина: ${e.reason}`)}),this.socket.addEventListener("message",async e=>{try{await W.setMessages(JSON.parse(e.data))}catch(r){H(r)}}),this.socket.addEventListener("error",e=>console.error("Ошибка",e))}async sendMessage(e){this.socket.send(JSON.stringify({content:e,type:"message"})),await W.getAllChats()}getOldMessages(){this.socket.send(JSON.stringify({content:"0",type:"get old"}))}_pingPong(){setInterval(()=>this.socket.send(JSON.stringify({type:"ping"})),1e4)}}const bo=(t,e)=>{let r=0;for(const s of t.concat(e))r^=s;return r};class Mo{async createChat(e){var r,s,c;if(!e){alert("Проверьте корректность введнных данных");return}try{if(await G.create({title:e}),S.state.chats){const p=(r=S.state.chats)==null?void 0:r.map(o=>o.id);await this.getAllChats();const d=(s=S.state.chats)==null?void 0:s.map(o=>o.id),h=bo(d,p);S.set("chat",(c=S.state.chats)==null?void 0:c.find(o=>o.id===h)),S.set("modals.isCreateChatModalOpen",!1),S.set("modals.isAddUserModalOpen",!0)}}catch(p){H(p)}}async getAllChats(){try{S.set("loading",!0);const e=await G.get();if(S.state.chats)S.set("chats",e.sort((r,s)=>{var c,p;return new Date((c=s==null?void 0:s.last_message)==null?void 0:c.time).getTime()-new Date((p=r==null?void 0:r.last_message)==null?void 0:p.time).getTime()}));else{const r=await Promise.all(e.map(async s=>({...s,socket:new Po({token:(await G.getToken(s.id)).token,chatId:s.id}),users:await G.getUsers(s.id)})));S.set("chats",r.sort((s,c)=>{var p,d;return new Date((p=c==null?void 0:c.last_message)==null?void 0:p.time).getTime()-new Date((d=s==null?void 0:s.last_message)==null?void 0:d.time).getTime()}))}}catch(e){H(e)}finally{S.set("loading",!1)}}async addUser(e){var r;if(!e){alert("Проверьте корректность введнных данных");return}if(!((r=S.state.chat)!=null&&r.id)){alert("Не выбран чат");return}try{const s=await we.search(e);if(s.length>1){alert("Введите логин пользователя полностью");return}await G.addUsers({users:[s[0].id],chatId:S.state.chat.id}),await this.getUsers(S.state.chat.id),S.set("modals.isAddUserModalOpen",!1)}catch(s){H(s)}}async removeUser(){var e,r;if(!((e=S.state.chat)!=null&&e.id)){alert("Не найден активный чат");return}if(!((r=S.state.chat)!=null&&r.toRemove)){alert("Не найден пользователь");return}try{await G.deleteUsers({chatId:S.state.chat.id,users:[S.state.chat.toRemove]}),await this.getUsers(S.state.chat.id),S.set("modals.isRemoveUserModalOpen",!1)}catch(s){H(s)}}async getUsers(e){if(e)try{const r=await G.getUsers(e);S.set("chat.users",r)}catch(r){H(r)}}sendMessage(e){var r,s;e&&((s=(r=S.state.chat)==null?void 0:r.socket)==null||s.sendMessage(e))}switchActiveChat(e){var s,c,p,d,h;let r=null;e!==((c=(s=S.state)==null?void 0:s.chat)==null?void 0:c.id)&&(r=e),S.set("chat",{...(p=S.state.chats)==null?void 0:p.find(o=>o.id===r),messages:[]}),r&&((h=(d=S.state.chat)==null?void 0:d.socket)==null||h.getOldMessages()),N.closeAll()}async changeChatAvatar(e){var r;try{const s=(r=S.state.chat)==null?void 0:r.id;if(s){const c=await G.uploadAvatar(s,e);N.closeAvatarLoad(),S.set("chat",c),await this.getAllChats()}}catch(s){console.error(s)}}async setMessages(e){var r,s;Array.isArray(e)?S.set("chat.messages",[...((r=S.state.chat)==null?void 0:r.messages)||[],...e.reverse()]):e.type==="message"&&S.set("chat.messages",[...((s=S.state.chat)==null?void 0:s.messages)||[],e]),await this.getAllChats()}async removeChat(){var r;const e=(r=S.state.chat)==null?void 0:r.id;if(e)try{S.set("chat",void 0),await G.delete(e),await this.getAllChats()}catch(s){H(s)}}}const W=new Mo;class xo extends b{constructor(e){super(e),this.props.openFilesMenu=N.switchFileMenu,this.props.onSend=r=>{r.preventDefault(),W.sendMessage(this.message.trim()),this.refs.messageForm.clear()}}get message(){return this.refs.messageForm.value}render(){return yo}}const ko=t=>{var e;return{isFilesMenuOpen:(e=t.modals)==null?void 0:e.isFilesMenuOpen}},Lo=D(xo,ko),Io=`
...
`;class Oo extends b{constructor(e){super(e),this.props.openChatMenu=N.switchChatMenu}render(){return Io}}const Ao=t=>{var e;return{isMembersOpen:(e=t.modals)==null?void 0:e.isMembersOpen}},Eo=D(Oo,Ao),No=`
...
`;class Bo extends b{constructor(e){super(e),this.props.events={click:async()=>W.switchActiveChat(this.props.id)}}render(){return No}}const Do=t=>{var e;return{openChatId:(e=t.chat)==null?void 0:e.id}},To=D(Bo,Do),Ro=`
  <ul class='chat-list'>
    {{# each chats }}
      {{{ ChatItem 
        id=this.id
...
`;class Fo extends b{render(){return Ro}}const Uo=t=>({chats:t.search&&t.chats?Object.values(t.chats).filter(e=>e.title.toLowerCase().includes((t.search||"").toLowerCase())):t.chats}),Ho=D(Fo,Uo),Vo=`
...
`;class qo extends b{get message(){return this.refs.chat.message}get newUserName(){return this.refs.chat.newUserName}render(){return Vo}}const zo=t=>{var e;return{openChatId:(e=t.chat)==null?void 0:e.id,chat:t.chat}},Wo=D(qo,zo),Go=`
...
`;class $o extends b{constructor(e){super(e),this.props.openCreateChatModal=N.switchCreateChat,this.props.newChatFields=[{type:"text",name:"chat_name"}],this.props.createChat=async r=>{r.preventDefault(),await W.createChat(this.refs.createChatModal.newChatName.trim())},this.props.onSearch=r=>{r.preventDefault(),N.setSearch(this.searchString)},this.props.goToProfile=()=>F.go(B.settings)}get searchString(){return this.refs.searchForm.value}render(){return Go}}const Ko=t=>{var e;return{search:t.search,isCreateChatModalOpen:(e=t.modals)==null?void 0:e.isCreateChatModalOpen}},Jo=D($o,Ko),Qo=`
...
`;class Zo extends b{get newChatName(){var e;return(e=this.refs.createChatModal)==null?void 0:e.values}get newUserName(){var e;return(e=this.refs.addUserModal)==null?void 0:e.values}render(){return Yo}}const Xo=`
  <div class='item'>
...
`;class ea extends b{constructor(e){super(e),this.props.onBlur=this.props.onBlur||(()=>this.isValid),this.props.validate=this.props.validate||(()=>!0)}get _value(){return this.refs.input.element.value}get rawValue(){return this._value}get value(){if(this.isValid)return this._value;console.error("Валидация не пройдена")}get isValid(){return this.props.validate(this._value,this.refs.errorText)}render(){return Xo}}const ta=t=>{var e,r;return{isEditProfileOpen:(e=t.modals)==null?void 0:e.isEditProfileOpen,isEditPasswordOpen:(r=t.modals)==null?void 0:r.isEditPasswordOpen}},ra=D(ea,ta),sa=`
...
`;class na extends b{constructor(e){super(e),this.props.emailValidate=vr,this.props.loginValidate=gt,this.props.passwordValidate=Me,this.props.repeatPasswordValidate=gr,this.props.nameValidate=ot,this.props.phoneValidate=_r}get values(){let e=!1;const r=Object.entries(this.refs).reduce((s,[c,p])=>(p.value||(e=!0),s[c]=p.value,s),{});return e?void 0:r}render(){return sa}}const ia=t=>{var e,r,s,c,p,d,h,o;return{isEditProfileOpen:(e=t.modals)==null?void 0:e.isEditProfileOpen,isEditPasswordOpen:(r=t.modals)==null?void 0:r.isEditPasswordOpen,login:(s=t.user)==null?void 0:s.login,email:(c=t.user)==null?void 0:c.email,phone:(p=t.user)==null?void 0:p.phone,first_name:(d=t.user)==null?void 0:d.first_name,second_name:(h=t.user)==null?void 0:h.second_name,display_name:(o=t.user)==null?void 0:o.display_name}},oa=D(na,ia),aa=`
...
`;class la extends b{render(){return aa}}const ca=t=>{var e,r;return{isEditProfileOpen:(e=t.modals)==null?void 0:e.isEditProfileOpen,isEditPasswordOpen:(r=t.modals)==null?void 0:r.isEditPasswordOpen}},ua=D(la,ca),ha=`
...
`;class pa{async changeProfile(e){if(!e){alert("Проверьте корректность введнных данных");return}try{const r=await we.changeProfile(e);S.set("user",r),S.set("modals.isEditProfileOpen",!1)}catch(r){H(r)}}async changeAvatar(e){if(e)try{const r=await we.changeAvatar(e);S.set("user",r)}catch(r){H(r)}}async changePassword(e){if(!e){alert("Проверьте корректность введнных данных");return}try{await we.changePassword(e),S.set("modals.isEditPasswordOpen",!1)}catch(r){H(r)}}}const at=new pa;class da extends b{constructor(e){super(e),this.props.onAvatarChange=async r=>{const s=r.target;if(s!=null&&s.files&&s.files.length>0){const c=new FormData;c.append("avatar",s.files[0]),await at.changeAvatar(c)}}}render(){return ha}}const fa=t=>{var e,r,s;return{isEditProfileOpen:(e=t.modals)==null?void 0:e.isEditProfileOpen,avatar:(r=t.user)==null?void 0:r.avatar,display_name:(s=t.user)==null?void 0:s.display_name}},ma=D(da,fa),ga=`
  <button class='{{ type }}-button{{# if className }} {{ className }}{{/ if }}'>
    {{ title }}
  </button>
`;class va extends b{constructor(e){super(e),this.props.events={click:this.props.onClick||(()=>{})}}render(){return ga}}const _a=`
  <input
    class='{{ className }}'
    name='{{ name }}'
    type='{{ type }}'
    ...
    ref='{{ ref }}'
  />
`;class ya extends b{constructor(e){super(e),this.props.events={blur:this.props.onBlur||(()=>{})}}render(){return _a}}const wa=`
...
`;class Ma extends b{constructor(e){super(e),this.props.events={submit:this.props.onSubmit||(r=>{r.preventDefault()})}}get value(){return Object.values(this.refs)[0].element.value}clear(){Object.values(this.refs)[0].element.value=""}render(){return ba}}const xa=`
  <div class='chat-info'>
...
`;class ka extends b{constructor(e){super(e),this.props.events={click:N.switchMembers}}render(){return xa}}const La=t=>{var e,r,s;return{users:(e=t.chat)==null?void 0:e.users,title:(r=t.chat)==null?void 0:r.title,avatar:(s=t.chat)==null?void 0:s.avatar}},Ia=D(ka,La),Oa=`
  <li class='menu__item menu__item_hover'>
    {{{ AddIcon className='icon_blue' }}}
    <span class='menu__title'>Добавить пользователя</span>
  </li>
`;class Aa extends b{constructor(e){super(e),this.props.events={click:()=>{N.switchAddUser(),N.switchMembers()}}}render(){return Oa}}const Ea=`
    <li class='menu__item'>
...
`;class Na extends b{constructor(e){super(e),this.props.events={click:()=>{N.setUserToRemove(this.props.id),N.switchRemoveUser(),N.switchMembers()}}}render(){return Ea}}const Ba=`
  {{{ IconButton
...
`;class Da extends b{constructor(e){super(e),this.props.events={click:this.props.onClick||(()=>{})}}render(){return Ba}}const Ta=`
  {{{ IconButton
...
`;class Ra extends b{constructor(e){super(e),this.props.events={click:this.props.onClick||(()=>{})}}render(){return Ta}}const Fa=`
  <input id='add_file' name='avatar' type='file' accept='image/*'/>
`;class Ua extends b{constructor(e){super(e),this.props.events={change:this.props.onChange||(()=>{})}}render(){return Fa}}class Ha extends b{render(){return cr}}class Va extends b{render(){return Xt}}class qa extends b{render(){return nr}}const za=`
  <svg class='icon {{ className }}' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 384 512'>
    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
    <path d='M0 64C0 28.7 28.7 0 64 0H224V128c0 17.7 14.3 32 32 32H384V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V64zm384 64H256V0L384 128z'/>
  </svg>
`;class Wa extends b{render(){return za}}class Ga extends b{render(){return ir}}const $a=`
  <svg class='icon {{ className }}' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 384 512'>
    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
...
`;class Ka extends b{render(){return $a}}const Ja=`
  <svg class='icon {{ className }}' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 448 512'>
    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
    <path d='M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.2 288 416 288c17.7 0 32-14.3 32-32s-14.3-32-32-32l-306.7 0L214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z'/>
  </svg>
`;class Qa extends b{render(){return Ja}}const ja=`
  <svg class='icon {{ className }}' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 384 512'>
    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
    <path d='M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z'/>
  </svg>
`;class Ya extends b{render(){return ja}}class Za extends b{render(){return sr}}class Xa extends b{render(){return or}}class el extends b{render(){return er}}class tl extends b{render(){return Zt}}class rl extends b{render(){return rr}}class sl extends b{render(){return ar}}const nl=`
  <svg class='icon {{ className }}' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 640 512'>
    <!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
...
`;class il extends b{render(){return nl}}class ol extends b{render(){return tr}}class al extends b{render(){return lr}}const ll=()=>{x("FilesInput",Ua),x("RemoveButton",Ra),x("UploadButton",Da),x("MembersRemoveItem",Na),x("MembersAddItem",Aa),x("ChatInfo",Ia),x("MenuItem",bi),x("PasswordItems",xi),x("Loader",Li),x("FormWithoutButtons",Ma),x("Button",va),x("IconButton",Oi),x("Input",ya),x("FormInput",Ei),x("Form",Bi),x("ErrorMessage",Ti),x("Avatar",Fi),x("Content",qi),x("ImagePreview",Wi),x("Info",Ji),x("ImageInput",ji),x("TextInput",Zi),x("MembersModal",ro),x("FilesMenu",no),x("MembersMenu",co),x("Message",fo),x("ChatBody",_o),x("Chat",Pa),x("ChatFooter",Lo),x("ChatHeader",Eo),x("ChatItem",To),x("ChatList",Ho),x("ChatSpace",Wo),x("ChatsPanel",Jo),x("WithoutChat",jo),x("Modal",Zo),x("ProfileItem",ra),x("ProfileBody",oa),x("ProfileFooter",ua),x("ProfileHeader",ma),x("AddIcon",Ha),x("EllipsisVerticalIcon",Va),x("ExitIcon",qa),x("FileIcon",Wa),x("HandIcon",Ga),x("ImageIcon",Ka),x("LeftArrowIcon",Qa),x("LocationIcon",Ya),x("LockIcon",Za),x("OtterIcon",Xa),x("PaperclipIcon",el),x("PenIcon",tl),x("PencilIcon",rl),x("PhotoIcon",sl),x("RemoveIcon",il),x("SearchIcon",ol),x("TrashIcon",al)},cl=`
...
`;class fl extends b{constructor(e){super(e),this.props.fields=[{label:"Почта",type:"email",name:"email",validate:vr,ref:"email"},{label:"Логин",type:"text",name:"login",validate:gt,ref:"login"},{label:"Имя",type:"text",name:"first_name",validate:ot,ref:"first_name"},{label:"Фамилия",type:"text",name:"second_name",validate:ot,ref:"second_name"},{label:"Телефон",type:"tel",name:"phone",validate:_r,ref:"phone"},{label:"Пароль",type:"password",name:"password",validate:Me,ref:"password"}],this.props.onSignup=r=>{r.preventDefault(),be.signup(this.refs.signupForm.values).catch(console.error)},this.props.goToLogin=()=>F.go(B.signin)}render(){return dl}}const ml=t=>({user:t.user,loading:t.loading}),gl=D(fl,ml),vl=`
...
`;let Dt=class extends b{constructor(e){super(e),this.props.goToChats=()=>F.go(B.messenger)}render(){return vl}};const _l=`
  <main class='wrapper wrapper_max wrapper_max'>
    {{{ ChatsPanel }}}
    {{{ ChatSpace }}}
  </main>
`;class yl extends b{constructor(e){super(e),W.getAllChats().catch(console.error)}render(){return _l}}const wl=`
...
`;class Cl extends b{constructor(e){super(e),this.props.logout=be.logout,this.props.onExit=N.switchExit,this.props.onEditProfile=N.switchEditProfile,this.props.onEditPassword=N.switchEditPassword,this.props.onSaveProfile=async r=>{r.preventDefault(),await at.changeProfile(this.refs.profileData.values)},this.props.onSavePassword=async r=>{r.preventDefault(),await at.changePassword(this.refs.profileData.values)},this.props.goToChats=()=>F.go(B.messenger),this.props.events={submit:this.props.onSave||(r=>r.preventDefault())}}render(){return wl}}const Sl=t=>{var e;return{isExitModalOpen:(e=t.modals)==null?void 0:e.isExitModalOpen,user:t.user,loading:t.loading}},Pl=D(Cl,Sl);On();vi();ll();document.addEventListener("DOMContentLoaded",async()=>{F.use(B.signin,pl).use(B.signup,gl).use(B.error,Dt,{code:500,message:"Уже фиксим",icon:"OtterIcon"}).use(B.error404,Dt,{code:404,message:"Такой страницы нет",icon:"HandIcon"}).use(B.messenger,yl).use(B.settings,Pl).start(),await S.init()});
